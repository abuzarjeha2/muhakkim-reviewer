...
export default function App() {
  ...
  const NAV = [
    ...
    { id: "predictor", name: "محرّك التنبؤ بالقرار", icon: Gauge },
    ...
    { id: "certificate", name: "شهادة الجاهزية", icon: BadgeCheck },
  ...
  return (
    <div dir="rtl" style={{ fontFamily: "Tajawal, system-ui, sans-serif", background: C.bg, minHeight: "100vh", display: "flex", color: C.navy }}>
      ...
      <main style={{ flex: 1, padding: "24px 32px", overflowX: "hidden" }}>
        ...
        {tab === "certificate" && <Certificate stats={stats} evaluations={evaluations} evidence={evidence} kpis={kpis} ssrSections={ssrSections} mockResults={mockResults} collegeInfo={collegeInfo} />}
...
// ============= 1. لوحة القيادة =============
function Dashboard({ stats, collegeInfo, setCollegeInfo, evaluations, evidence, kpis, ssrSections, mockResults, setTab }) {
  const conf = useMemo(() => computeScores(stats, evaluations, evidence, kpis, ssrSections, mockResults), [stats, evaluations, evidence, kpis, ssrSections, mockResults]);
  const pred = useMemo(() => computeReadiness(stats, evaluations, evidence, kpis), [stats, evaluations, evidence, kpis]);
  const cBand = conf.overall >= 85 ? "#16a34a" : conf.overall >= 65 ? "#f59e0b" : "#dc2626";
  const pBand = pred.probability >= 80 ? "#16a34a" : pred.probability >= 60 ? "#f59e0b" : "#dc2626";
  ...
  return (
    <div>
      ...
      {/* البطاقة البارزة: الثقة والتنبؤ */}
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 20 }}>
        {[
          { title: "درجة الثقة الإجمالية", val: conf.overall, color: cBand, sub: conf.overall >= 85 ? "موثوقية عالية" : conf.overall >= 65 ? "موثوقية متوسطة" : "تحتاج تحسيناً", tab: "certificate", cta: "عرض الشهادة" },
          { title: "احتمال الحصول على الاعتماد", val: pred.probability, color: pBand, sub: pred.probability >= 80 ? "احتمال مرتفع" : pred.probability >= 60 ? "احتمال متوسط" : "احتمال منخفض", tab: "predictor", cta: "تحليل المخاطر" },
        ].map((b, i) => {
          const off = circ * (1 - b.val / 100);
          return (
            <Card key={i} style={{ background: `linear-gradient(135deg,${C.navy},${C.navy2})`, color: "#fff", display: "flex", alignItems: "center", gap: 20 }}>
              <svg width={130} height={130} style={{ transform: "rotate(-90deg)", flexShrink: 0 }}>
                <circle cx={65} cy={65} r={R} fill="none" stroke="rgba(255,255,255,.12)" strokeWidth={12} />
                <circle cx={65} cy={65} r={R} fill="none" stroke={b.color} strokeWidth={12} strokeLinecap="round" strokeDasharray={circ} strokeDashoffset={off} style={{ transition: "stroke-dashoffset 1s ease" }} />
                <text x={65} y={60} textAnchor="middle" fill="#fff" fontSize={26} fontWeight={800} style={{ transform: "rotate(90deg)", transformOrigin: "65px 65px" }}>{b.val}%</text>
              </svg>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 14, color: C.goldL }}>{b.title}</div>
                <div style={{ fontSize: 20, fontWeight: 800, color: b.color, margin: "4px 0 12px" }}>{b.sub}</div>
                <Btn variant="gold" onClick={() => setTab(b.tab)} style={{ padding: "8px 16px", fontSize: 13 }}>{b.cta} <ChevronRight size={15} /></Btn>
              </div>
            </Card>
          );
        })}
  ...
  return (
    <div>
      ...
      {Object.keys(mockResults).length > 0 && (() => {
        const all = Object.values(mockResults);
        const overall = (all.reduce((a, r) => a + (r.readiness || 0), 0) / all.length);
        const cellFor = (sid, rid) => mockResults[`${sid}_${rid}`]?.readiness;
        return (
          <Card style={{ marginBottom: 20 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16, flexWrap: "wrap", gap: 10 }}>
              <h3 style={{ margin: 0 }}>لوحة الجاهزية الشاملة</h3>
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <span style={{ fontSize: 13, color: "#64748b" }}>متوسط الجاهزية العام</span>
                <span style={{ fontSize: 22, fontWeight: 800, color: COMPLIANCE_COLORS[Math.round(overall)] }}>{overall.toFixed(2)}/4</span>
              </div>
            </div>
            <div style={{ overflowX: "auto" }}>
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13, minWidth: 520 }}>
                <thead><tr><th style={{ ...th, minWidth: 180 }}>المعيار</th>{ROLES.map(r => <th key={r.id} style={{ ...th, textAlign: "center" }}>{r.name}</th>)}<th style={{ ...th, textAlign: "center" }}>المتوسط</th></tr></thead>
                <tbody>
                  {STANDARDS.map(s => {
                    const cells = ROLES.map(r => cellFor(s.id, r.id));
                    const valid = cells.filter(c => c != null);
                    const avg = valid.length ? (valid.reduce((a, b) => a + b, 0) / valid.length) : null;
                    return (
                      <tr key={s.id}>
                        <td style={{ ...td, fontWeight: 600 }}>{s.code}. {s.name}</td>
                        {cells.map((c, i) => (
                          <td key={i} style={{ ...td, textAlign: "center" }}>
                            {c != null ? <span style={{ display: "inline-block", width: 30, height: 30, lineHeight: "30px", borderRadius: 8, background: COMPLIANCE_COLORS[c], color: "#fff", fontWeight: 800 }}>{c}</span> : <span style={{ color: "#cbd5e1" }}>—</span>}
                          </td>
                        ))}
                        <td style={{ ...td, textAlign: "center", fontWeight: 800, color: avg != null ? COMPLIANCE_COLORS[Math.round(avg)] : "#cbd5e1" }}>{avg != null ? avg.toFixed(1) : "—"}</td>
                      </tr>
                    );
                  })}
                </tbody>
...
// ============= محرّك التنبؤ بقرار الاعتماد =============
// يحسب الاحتمال حسابياً من البيانات الفعلية ثم يستعين بالذكاء لتحليل المخاطر
function computeReadiness(stats, evaluations, evidence, kpis) {
  const allCriteria = STANDARDS.flatMap(s => s.criteria);
  // المعايير الجوهرية (Essential) — وزنها أعلى في قرار المراجع
  const essential = ["1.0.3","2.1.2","2.2.1","2.4.3","2.4.4","2.5.1","3.1.1","3.1.2","3.3.1","3.5.1","3.6.1","4.1.1","4.5.1","5.1.3","5.2.4","6.2.1","7.1.1","7.1.5"];

  let factors = [];

  // 1) متوسط التقييم العام (الوزن الأكبر 40%)
  const scored = allCriteria.map(c => evaluations[c]?.score || 0).filter(x => x > 0);
  const avgScore = scored.length ? scored.reduce((a, b) => a + b, 0) / scored.length : 0;
  const scoreFactor = avgScore / 4; // 0-1
  factors.push({ key: "متوسط التقييم", weight: 0.40, value: scoreFactor, raw: avgScore.toFixed(2) });

  // 2) المعايير الجوهرية التي درجتها <3 (خطر مرتفع على القرار)
  const failingEssential = essential.filter(c => (evaluations[c]?.score || 0) > 0 && evaluations[c].score < 3);
  const unscoredEssential = essential.filter(c => !(evaluations[c]?.score > 0));
  const essOk = essential.filter(c => (evaluations[c]?.score || 0) >= 3).length;
  const essFactor = essential.length ? essOk / essential.length : 0;
  factors.push({ key: "المعايير الجوهرية", weight: 0.25, value: essFactor, raw: `${essOk}/${essential.length}` });

  // 3) تغطية الأدلة (نسبة المعايير التي لها دليل واحد على الأقل)
  const covered = new Set(evidence.map(e => e.criterion));
  const coverFactor = allCriteria.filter(c => covered.has(c)).length / allCriteria.length;
  factors.push({ key: "تغطية الأدلة", weight: 0.15, value: coverFactor, raw: Math.round(coverFactor * 100) + "%" });

  // 4) جودة الأدلة المراجعة
  const reviewed = evidence.filter(e => e.reviewed && e.quality);
  const qualFactor = reviewed.length ? (reviewed.reduce((a, e) => a + e.quality, 0) / reviewed.length) / 5 : 0.5;
  factors.push({ key: "جودة الأدلة", weight: 0.10, value: qualFactor, raw: reviewed.length ? (reviewed.reduce((a, e) => a + e.quality, 0) / reviewed.length).toFixed(1) + "/5" : "لم تُراجع" });

  // 5) تحقق المؤشرات (نسبة الـ KPIs التي بلغت المستهدف)
  const kpiMet = kpis.length ? kpis.filter(k => k.actual >= k.target).length / kpis.length : 0.5;
  factors.push({ key: "تحقق المؤشرات", weight: 0.10, value: kpiMet, raw: kpis.length ? `${kpis.filter(k => k.actual >= k.target).length}/${kpis.length}` : "—" });

  const score01 = factors.reduce((a, f) => a + f.weight * f.value, 0);
  let probability = Math.round(score01 * 100);
  // عقوبة على كل معيار جوهري ساقط
  probability = Math.max(0, probability - failingEssential.length * 6 - unscoredEssential.length * 2);
  probability = Math.min(99, probability);

  return { probability, factors, avgScore, failingEssential, unscoredEssential, coverFactor };
}

function Predictor({ stats, evaluations, evidence, kpis, collegeInfo, setTab }) {
  const [analysis, setAnalysis] = useState(null);
  const [loading, setLoading] = useState(false);
  const calc = useMemo(() => computeReadiness(stats, evaluations, evidence, kpis), [stats, evaluations, evidence, kpis]);

  const band = calc.probability >= 80 ? { label: "احتمال مرتفع للاعتماد", color: "#16a34a", desc: "وضعك قوي. ركّز على إغلاق الثغرات الأخيرة." }
    : calc.probability >= 60 ? { label: "احتمال متوسط — قابل للتحسين", color: "#f59e0b", desc: "أساس جيد، لكن توجد مخاطر تحتاج معالجة قبل الزيارة." }
    : { label: "احتمال منخفض — يتطلب عملاً عاجلاً", color: "#dc2626", desc: "مخاطر جوهرية قد تهدد القرار. عالجها فوراً." };

  const runAnalysis = async () => {
    setLoading(true);
    const stdSummary = stats.standardAverages.map(s => `${s.name}: ${s.avg > 0 ? s.avg.toFixed(2) : "غير مقيّم"}`).join("، ");
    const factorSummary = calc.factors.map(f => `${f.key}: ${f.raw} (وزن ${Math.round(f.weight * 100)}%)`).join("، ");
    const res = await callAIJson(
      `أنت خبير اعتماد أكاديمي NCAAA. لديّ مؤسسة "${collegeInfo.name}". احتمال الاعتماد المحسوب حالياً ${calc.probability}%.\n` +
      `عوامل الحساب: ${factorSummary}.\n` +
      `متوسطات المعايير: ${stdSummary}.\n` +
      `معايير جوهرية درجتها أقل من 3: ${calc.failingEssential.join(", ") || "لا يوجد"}.\n` +
      `معايير جوهرية غير مقيّمة: ${calc.unscoredEssential.join(", ") || "لا يوجد"}.\n` +
      `حدد أكبر 4 مخاطر تهدد قرار الاعتماد مرتبة حسب الأثر، ولكل خطر: المعيار المرتبط، الإجراء التصحيحي، وكم نقطة مئوية تقريباً يرفعها معالجته.`,
      `أرجع JSON: {"risks":[{"title":"عنوان الخطر","criterion":"رقم المعيار","impact":نقاط مئوية رقم,"action":"الإجراء التصحيحي","evidenceNeeded":"الدليل الناقص المطلوب"}],"verdict":"حكم موجز كأنه من مراجع خارجي","potentialProbability":النسبة المتوقعة بعد المعالجة رقم}`
    );
    setAnalysis(res);
    setLoading(false);
  };
  ...
  // مؤشر دائري
  const R = 80, circ = 2 * Math.PI * R, off = circ * (1 - calc.probability / 100);

  return (
    <div>
      <Header icon={Gauge} title="محرّك التنبؤ بقرار الاعتماد" sub="يحسب احتمال حصولك على الاعتماد من بياناتك الفعلية، ويكشف أكبر المخاطر المؤثرة على القرار" />
      ...
      <div style={{ display: "grid", gridTemplateColumns: "320px 1fr", gap: 16, marginBottom: 16 }}>
        <Card style={{ textAlign: "center", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
          <svg width={200} height={200} style={{ transform: "rotate(-90deg)" }}>
            <circle cx={100} cy={100} r={R} fill="none" stroke="#eef1f6" strokeWidth={16} />
            <circle cx={100} cy={100} r={R} fill="none" stroke={band.color} strokeWidth={16} strokeLinecap="round" strokeDasharray={circ} strokeDashoffset={off} style={{ transition: "stroke-dashoffset 1s ease" }} />
          </svg>
          <div style={{ marginTop: -130, marginBottom: 70 }}>
            <div style={{ fontSize: 44, fontWeight: 800, color: band.color }}>{calc.probability}%</div>
            <div style={{ fontSize: 12, color: "#64748b" }}>احتمال الاعتماد</div>
          </div>
          <div style={{ fontWeight: 700, color: band.color, fontSize: 15 }}>{band.label}</div>
          <div style={{ fontSize: 13, color: "#64748b", marginTop: 6, lineHeight: 1.6 }}>{band.desc}</div>
        </Card>

        <Card>
          <h3 style={{ marginTop: 0, display: "flex", alignItems: "center", gap: 8 }}><Target size={20} color={C.gold} /> عوامل حساب الاحتمال</h3>
          <p style={{ fontSize: 13, color: "#64748b", marginTop: -4 }}>الاحتمال محسوب فعلياً من بياناتك (ليس تقديراً عشوائياً) — كل عامل بوزنه:</p>
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {calc.factors.map((f, i) => (
              <div key={i}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4, fontSize: 14 }}>
                  <span style={{ fontWeight: 600 }}>{f.key} <span style={{ color: "#94a3b8", fontSize: 12 }}>(وزن {Math.round(f.weight * 100)}%)</span></span>
                  <span style={{ fontWeight: 700, color: C.navy }}>{f.raw}</span>
                </div>
                <div style={{ height: 8, background: "#eef1f6", borderRadius: 4 }}><div style={{ height: "100%", width: (f.value * 100) + "%", background: f.value >= .75 ? "#16a34a" : f.value >= .5 ? "#f59e0b" : "#dc2626", borderRadius: 4, transition: ".5s" }} /></div>
              </div>
            ))}
          </div>
          {(calc.failingEssential.length > 0 || calc.unscoredEssential.length > 0) && (
            <div style={{ marginTop: 14, padding: 12, background: "#fef2f2", borderRadius: 10, fontSize: 13, color: "#dc2626" }}>
              <ShieldAlert size={15} style={{ verticalAlign: "middle", marginLeft: 4 }} />
              {calc.failingEssential.length > 0 && <span> {calc.failingEssential.length} معيار جوهري درجته أقل من 3.</span>}
              {calc.unscoredEssential.length > 0 && <span> {calc.unscoredEssential.length} معيار جوهري غير مقيّم.</span>}
              {" "}المعايير الجوهرية تؤثر بقوة على القرار.
            </div>
          )}
        </Card>
      </div>
      ...
      <Card style={{ marginBottom: 16 }}>
        <Btn onClick={runAnalysis} disabled={loading}>{loading ? <Loader2 size={18} style={{ animation: "spin 1s linear infinite" }} /> : <Sparkles size={18} />} تحليل المخاطر المؤثرة على القرار بالذكاء الاصطناعي</Btn>
      </Card>

      {analysis && (
        <>
          {analysis.verdict && (
            <Card style={{ marginBottom: 16, background: `linear-gradient(135deg,${C.navy},${C.navy2})`, color: "#fff" }}>
              <div style={{ fontSize: 13, color: C.goldL, marginBottom: 6 }}>حكم المراجع الافتراضي</div>
              <div style={{ fontSize: 15, lineHeight: 1.8 }}>{analysis.verdict}</div>
              {analysis.potentialProbability && (
                <div style={{ marginTop: 14, display: "flex", alignItems: "center", gap: 12, background: "rgba(255,255,255,.08)", padding: 14, borderRadius: 12 }}>
                  <span style={{ fontSize: 13 }}>الاحتمال الحالي</span>
                  <span style={{ fontSize: 22, fontWeight: 800, color: band.color }}>{calc.probability}%</span>
                  <ArrowUp size={20} color={C.goldL} />
                  <span style={{ fontSize: 13 }}>بعد معالجة المخاطر</span>
                  <span style={{ fontSize: 22, fontWeight: 800, color: "#4ade80" }}>{analysis.potentialProbability}%</span>
                </div>
              )}
            </Card>
          )}
          <h3 style={{ display: "flex", alignItems: "center", gap: 8 }}><ShieldAlert size={20} color="#dc2626" /> أكبر المخاطر المؤثرة على قرار الاعتماد</h3>
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {(analysis.risks || []).map((r, i) => (
              <Card key={i} style={{ borderRight: "4px solid #dc2626" }}>
                <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
                  <div style={{ width: 30, height: 30, borderRadius: 8, background: "#fef2f2", color: "#dc2626", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 800, flexShrink: 0 }}>{i + 1}</div>
                  <div style={{ flex: 1 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
                      <span style={{ fontWeight: 700, fontSize: 15 }}>{r.title}</span>
                      <span style={{ fontSize: 12, fontWeight: 700, background: "#dcfce7", color: "#16a34a", padding: "4px 10px", borderRadius: 8 }}>+{r.impact}% عند المعالجة</span>
                    </div>
                    {r.criterion && <span style={{ display: "inline-block", marginTop: 6, fontSize: 11, background: C.navy, color: C.goldL, padding: "3px 8px", borderRadius: 6 }}>المعيار {r.criterion}</span>}
                    <div style={{ marginTop: 10, fontSize: 14, lineHeight: 1.7 }}><strong style={{ color: C.navy }}>الإجراء التصحيحي: </strong>{r.action}</div>
                    {r.evidenceNeeded && <div style={{ marginTop: 6, fontSize: 13, color: "#64748b" }}><strong style={{ color: C.gold }}>الدليل الناقص: </strong>{r.evidenceNeeded}</div>}
                  </div>
                </div>
              </Card>
            ))}
...
// ============= شهادة الجاهزية ودرجة الثقة =============
function computeScores(stats, evaluations, evidence, kpis, ssrSections, mockResults) {
  const allCriteria = STANDARDS.flatMap(s => s.criteria);
  // الجودة: متوسط التقييم العام على 100
  const scored = allCriteria.map(c => evaluations[c]?.score || 0).filter(x => x > 0);
  const quality = scored.length ? Math.round((scored.reduce((a, b) => a + b, 0) / scored.length) / 4 * 100) : 0;
  // المنهجية: نسبة المعايير المقيّمة + اكتمال أقسام SSR
  const evalPct = stats.evaluated / stats.totalCriteria;
  const ssrPct = Object.keys(ssrSections).filter(k => ssrSections[k]).length / 9;
  const methodology = Math.round((evalPct * 0.6 + ssrPct * 0.4) * 100);
  // الأدلة: تغطية × جودة المراجعة
  const covered = new Set(evidence.map(e => e.criterion));
  const coverPct = allCriteria.filter(c => covered.has(c)).length / allCriteria.length;
  const reviewed = evidence.filter(e => e.reviewed && e.quality);
  const qualAvg = reviewed.length ? (reviewed.reduce((a, e) => a + e.quality, 0) / reviewed.length) / 5 : 0.5;
  const evidenceScore = Math.round((coverPct * 0.6 + qualAvg * 0.4) * 100);
  // المؤشرات: نسبة تحقق المستهدف
  const kpiScore = kpis.length ? Math.round(kpis.filter(k => k.actual >= k.target).length / kpis.length * 100) : 0;
  // اللغة والتوثيق: اكتمال التعليقات على المعايير المقيّمة
  const withComment = allCriteria.filter(c => evaluations[c]?.score > 0 && (evaluations[c]?.comment || "").trim().length > 20).length;
  const language = scored.length ? Math.round(withComment / scored.length * 100) : 0;

  const scores = [
    { key: "درجة الجودة", val: quality },
    { key: "درجة المنهجية", val: methodology },
    { key: "درجة الأدلة", val: evidenceScore },
    { key: "درجة المؤشرات", val: kpiScore },
    { key: "درجة اللغة والتوثيق", val: language },
  ];
  const overall = Math.round(scores.reduce((a, s) => a + s.val, 0) / scores.length);
  const verified = {
    "المنهجية": evalPct >= 0.9,
    "النتائج": kpis.length > 0,
    "المراجع": Object.keys(ssrSections).filter(k => ssrSections[k]).length >= 5,
    "الاتساق": coverPct >= 0.7,
    "اللغة": language >= 50,
    "المؤشرات": kpiScore >= 60,
  };
  return { scores, overall, verified, sourceCount: evidence.length };
}

function Certificate({ stats, evaluations, evidence, kpis, ssrSections, mockResults, collegeInfo }) {
  const [aiVerdict, setAiVerdict] = useState(null);
  const [loading, setLoading] = useState(false);
  const sc = useMemo(() => computeScores(stats, evaluations, evidence, kpis, ssrSections, mockResults), [stats, evaluations, evidence, kpis, ssrSections, mockResults]);
  const certId = useMemo(() => "MHK-" + Math.abs(hashStr(collegeInfo.name + sc.overall + stats.totalCriteria)).toString(36).toUpperCase().slice(0, 8), [collegeInfo.name, sc.overall, stats.totalCriteria]);

  const band = sc.overall >= 85 ? { l: "موثوقية عالية", c: "#16a34a" } : sc.overall >= 65 ? { l: "موثوقية متوسطة", c: "#f59e0b" } : { l: "تحتاج تحسيناً", c: "#dc2626" };

  const verify = async () => {
    setLoading(true);
    const summary = sc.scores.map(s => `${s.key}: ${s.val}%`).join("، ");
    const res = await callAI(`أنت مدقق جودة في الاعتماد الأكاديمي. درجة الثقة الإجمالية لمؤسسة "${collegeInfo.name}" هي ${sc.overall}% بناءً على: ${summary}. اكتب فقرة قصيرة (٣ أسطر) تلخّص مستوى موثوقية جاهزية الدراسة الذاتية وأبرز ما يدعمها أو يضعفها، بأسلوب رسمي.`);
    setAiVerdict(res);
    setLoading(false);
  };

  const exportCert = () => {
    const rows = sc.scores.map(s => `<tr><td style="padding:8px;border-bottom:1px solid #eee">${s.key}</td><td style="padding:8px;border-bottom:1px solid #eee;text-align:left;font-weight:bold;color:#0f1f3d">${s.val}%</td></tr>`).join("");
    const html = `<html dir="rtl"><head><meta charset="utf-8"><style>body{font-family:Tajawal,Arial;padding:0;margin:0;background:#f4f6fb}.cert{max-width:760px;margin:30px auto;background:#fff;border:3px solid #c9a227;border-radius:18px;padding:40px;text-align:center}h1{color:#c9a227;margin:6px 0}.score{font-size:64px;font-weight:800;color:${band.c}}table{width:100%;border-collapse:collapse;margin:20px 0;text-align:right}.id{font-family:monospace;background:#f8f9fc;padding:6px 14px;border-radius:8px;display:inline-block;color:#0f1f3d}</style></head><body><div class="cert">
      <div style="font-size:40px;color:#c9a227">★</div>
      <div style="color:#64748b;font-size:14px">Mohakam AI Certified Report — شهادة جاهزية محكّم</div>
      <h1>${collegeInfo.name}</h1>
      ${collegeInfo.program ? `<div style="color:#64748b">${collegeInfo.program}</div>` : ""}
      <div style="color:#64748b;margin-top:10px">درجة الثقة الإجمالية</div>
      <div class="score">${sc.overall}%</div>
      <div style="font-weight:bold;color:${band.c}">${band.l}</div>
      <table>${rows}</table>
      ${aiVerdict ? `<p style="color:#475569;line-height:1.8;font-size:14px;text-align:right">${aiVerdict}</p>` : ""}
      <div style="margin-top:20px"><span class="id">معرّف الشهادة: ${certId}</span></div>
      <div style="color:#94a3b8;font-size:12px;margin-top:10px">صادرة بتاريخ ${new Date().toLocaleDateString("ar-SA")} · عدد الأدلة الداعمة: ${sc.sourceCount}</div>
    </div></body></html>`;
    const blob = new Blob([html], { type: "text/html" }); const a = document.createElement("a"); a.href = URL.createObjectURL(blob); a.download = `Mohakam_Certificate_${certId}.html`; a.click();
  };

  const R = 70, circ = 2 * Math.PI * R, off = circ * (1 - sc.overall / 100);

  return (
    <div>
      <Header icon={BadgeCheck} title="شهادة جاهزية محكّم AI ودرجة الثقة" sub="درجة ثقة شاملة محسوبة من بياناتك الفعلية، مع شهادة رقمية قابلة للتصدير والتحقق" />

      <div style={{ display: "grid", gridTemplateColumns: "300px 1fr", gap: 16, marginBottom: 16 }}>
        <Card style={{ textAlign: "center", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
          <svg width={180} height={180} style={{ transform: "rotate(-90deg)" }}>
            <circle cx={90} cy={90} r={R} fill="none" stroke="#eef1f6" strokeWidth={14} />
            <circle cx={90} cy={90} r={R} fill="none" stroke={band.c} strokeWidth={14} strokeLinecap="round" strokeDasharray={circ} strokeDashoffset={off} style={{ transition: "stroke-dashoffset 1s ease" }} />
          </svg>
          <div style={{ marginTop: -118, marginBottom: 58 }}>
            <div style={{ fontSize: 40, fontWeight: 800, color: band.c }}>{sc.overall}%</div>
            <div style={{ fontSize: 12, color: "#64748b" }}>درجة الثقة</div>
          </div>
          <div style={{ fontWeight: 700, color: band.c }}>{band.l}</div>
          <div style={{ marginTop: 8, fontSize: 11, fontFamily: "monospace", background: "#f8f9fc", padding: "4px 10px", borderRadius: 8 }}>{certId}</div>
        </Card>

        <Card>
          <h3 style={{ marginTop: 0, display: "flex", alignItems: "center", gap: 8 }}><ShieldCheck size={20} color={C.gold} /> بطاقة الدرجات</h3>
          <p style={{ fontSize: 13, color: "#64748b", marginTop: -4 }}>كل درجة محسوبة من بياناتك الفعلية في المنصة:</p>
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {sc.scores.map((s, i) => (
              <div key={i}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4, fontSize: 14 }}>
                  <span style={{ fontWeight: 600 }}>{s.key}</span>
                  <span style={{ fontWeight: 800, color: s.val >= 75 ? "#16a34a" : s.val >= 50 ? "#f59e0b" : "#dc2626" }}>{s.val}%</span>
                </div>
                <div style={{ height: 8, background: "#eef1f6", borderRadius: 4 }}><div style={{ height: "100%", width: s.val + "%", background: s.val >= 75 ? "#16a34a" : s.val >= 50 ? "#f59e0b" : "#dc2626", borderRadius: 4, transition: ".5s" }} /></div>
              </div>
            ))}
          </div>
          <div style={{ marginTop: 16, display: "flex", flexWrap: "wrap", gap: 8 }}>
            {Object.entries(sc.verified).map(([k, v]) => (
              <span key={k} style={{ fontSize: 12, fontWeight: 600, padding: "5px 12px", borderRadius: 20, display: "flex", alignItems: "center", gap: 5, background: v ? "#dcfce7" : "#f1f5f9", color: v ? "#16a34a" : "#94a3b8" }}>
                {v ? <CheckCircle2 size={13} /> : <X size={13} />} تم التحقق من {k}
              </span>
            ))}
          </div>
        </Card>
      ...
      <Card style={{ marginBottom: 16, display: "flex", gap: 10, flexWrap: "wrap" }}>
        <Btn onClick={verify} disabled={loading}>{loading ? <Loader2 size={18} style={{ animation: "spin 1s linear infinite" }} /> : <Sparkles size={18} />} توليد حكم الموثوقية بالذكاء</Btn>
        <Btn variant="gold" onClick={exportCert}><Download size={18} /> تصدير الشهادة الرقمية</Btn>
      </Card>

      {aiVerdict && <Card style={{ marginBottom: 16, borderRight: `4px solid ${C.gold}` }}><div style={{ fontSize: 13, color: C.gold, fontWeight: 700, marginBottom: 6 }}>حكم الموثوقية</div><div style={{ lineHeight: 1.8, fontSize: 15 }}>{aiVerdict}</div></Card>}

      {/* معاينة الشهادة */}
      <Card style={{ border: `3px solid ${C.gold}`, textAlign: "center", padding: 36 }}>
        <div style={{ fontSize: 36, color: C.gold }}>★</div>
        <div style={{ color: "#64748b", fontSize: 13 }}>Mohakam AI Certified Report — شهادة جاهزية محكّم</div>
        <h2 style={{ color: C.navy, margin: "8px 0" }}>{collegeInfo.name}</h2>
        {collegeInfo.program && <div style={{ color: "#64748b" }}>{collegeInfo.program}</div>}
        <div style={{ fontSize: 56, fontWeight: 800, color: band.c, marginTop: 10 }}>{sc.overall}%</div>
        <div style={{ fontWeight: 700, color: band.c }}>{band.l}</div>
        <div style={{ display: "flex", justifyContent: "center", gap: 18, flexWrap: "wrap", marginTop: 20 }}>
          {sc.scores.map((s, i) => <div key={i} style={{ textAlign: "center" }}><div style={{ fontSize: 22, fontWeight: 800, color: C.navy }}>{s.val}%</div><div style={{ fontSize: 11, color: "#94a3b8" }}>{s.key.replace("درجة ", "")}</div></div>)}
        </div>
        <div style={{ marginTop: 24, display: "inline-flex", alignItems: "center", gap: 14, background: "#f8f9fc", padding: "12px 20px", borderRadius: 12 }}>
          <div style={{ width: 64, height: 64, background: "#fff", borderRadius: 8, display: "grid", gridTemplateColumns: "repeat(5,1fr)", gridTemplateRows: "repeat(5,1fr)", gap: 1, padding: 4, border: "1px solid #e2e8f0" }}>
            {Array.from({ length: 25 }).map((_, i) => <div key={i} style={{ background: (hashStr(certId + i) % 2 === 0) ? C.navy : "transparent" }} />)}
          </div>
          <div style={{ textAlign: "right" }}>
            <div style={{ fontSize: 11, color: "#94a3b8" }}>للتحقق · معرّف الشهادة</div>
            <div style={{ fontFamily: "monospace", fontWeight: 700, color: C.navy }}>{certId}</div>
          </div>
        </div>
      </Card>
    </div>
