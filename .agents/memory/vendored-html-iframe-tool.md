...
**Making a demo tool's fake AI real:** these tools usually fake "AI" with `setTimeout` + hardcoded strings / `alert()` / `Math.random()`. To make them actually work, inject ONE shared helper per file — `callAI(system,user,maxTokens)` that `fetch("/api/ai")` (body `{model:"claude-sonnet-4-6",max_tokens,system,messages:[{role:"user",content}]}`), `extractAIText(d)` over `d.content[].text`, and a tiny `aiMarkdown()` that HTML-escapes first then formats — then make each handler `async`, show a loading state, `await callAI(...)`, render via `aiMarkdown` into a visible element (replace every `alert()` with a result `<div>`), wrapped in try/catch. `gov-platform.html` is the canonical reference. This is highly parallelizable: one delegated subagent per file works well. Verify each file has exactly one `callAI` def (`rg -c`) and the page still loads (console init message, no "already declared").
