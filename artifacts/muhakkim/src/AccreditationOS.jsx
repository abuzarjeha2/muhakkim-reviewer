...
export function AccreditationOS() {
  ...
  return (
    <div dir="rtl" style={{ position: "relative", fontFamily: "Tajawal, system-ui, sans-serif", background: C.bg, minHeight: "calc(100vh - 120px)", display: "flex", color: C.navy, borderRadius: 16, overflow: "hidden", border: "1px solid #eef1f6" }}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Tajawal:wght@400;500;700;800&display=swap');
        .acc-os *{box-sizing:border-box} .acc-os ::-webkit-scrollbar{width:8px;height:8px} .acc-os ::-webkit-scrollbar-thumb{background:${C.gold};border-radius:4px}
        .acc-os .glass{background:rgba(255,255,255,.9);backdrop-filter:blur(10px)} .acc-os .fade{animation:accf .4s ease} @keyframes accf{from{opacity:0;transform:translateY(8px)}to{opacity:1}}
        @keyframes accspin{to{transform:rotate(360deg)}}`}</style>

      <div className="acc-os" style={{ display: "flex", width: "100%" }}>
      {!loaded && (
        <div style={{ position: "absolute", inset: 0, background: `linear-gradient(135deg,${C.navy},${C.navy2})`, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", zIndex: 50, gap: 16, color: "#fff", borderRadius: 16 }}>
          ...
          <Loader2 size={28} color={C.goldL} style={{ animation: "accspin 1s linear infinite" }} />
          <div style={{ color: C.goldL, fontSize: 14 }}>جاري تحميل بياناتك المحفوظة...</div>
        </div>
      )}

      {/* الشريط الجانبي */}
      <aside style={{ width: 260, background: `linear-gradient(180deg,${C.navy},${C.navy2})`, color: "#fff", padding: "20px 0", maxHeight: "calc(100vh - 120px)", overflowY: "auto", flexShrink: 0 }}>
        ...
        <div style={{ padding: 16, margin: "16px 12px 0", background: "rgba(201,162,39,.1)", borderRadius: 12, fontSize: 12, color: "#cdd5e0" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 12, fontSize: 11 }}>
            {saveStatus === "saving" && <><Loader2 size={13} style={{ animation: "accspin 1s linear infinite" }} /> جاري الحفظ...</>}
          ...
          {evidence.map(e => (
            <Card key={e.id}>
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: e.reviewed ? 12 : 0 }}>
                ...
                <Btn variant="ghost" onClick={() => reviewOne(e)} disabled={loading === e.id} style={{ padding: "6px 12px", fontSize: 13 }}>{loading === e.id ? <Loader2 size={16} style={{ animation: "accspin 1s linear infinite" }} /> : <Sparkles size={15} />} مراجعة</Btn>
              </div>
...
function GapAnalysis({ stats, evaluations }) {
  ...
  return (
    <div>
      ...
      <Card style={{ marginBottom: 16 }}><Btn onClick={analyze} disabled={loading}>{loading ? <Loader2 size={18} style={{ animation: "accspin 1s linear infinite" }} /> : <Sparkles size={18} />} تحليل الفجوات بالذكاء الاصطناعي</Btn></Card>
      {result && (
  ...
  return (
    <div>
      ...
      <Card style={{ marginBottom: 16 }}>
        ...
        <div style={{ marginTop: 12 }}><Btn variant="gold" onClick={run} disabled={loading}>{loading ? <Loader2 size={18} style={{ animation: "accspin 1s linear infinite" }} /> : <Sparkles size={18} />} معالجة النص</Btn></div>
      </Card>
        ...
        {sections.map(sec => (
          <Card key={sec.id}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: ssrSections[sec.id] ? 12 : 0 }}>
              ...
              <Btn variant="ghost" onClick={() => generate(sec)} disabled={loading === sec.id} style={{ padding: "6px 14px", fontSize: 13 }}>{loading === sec.id ? <Loader2 size={15} style={{ animation: "accspin 1s linear infinite" }} /> : <Sparkles size={15} />} توليد</Btn>
            </div>
  ...
  return (
    <div>
      ...
      <Card style={{ marginBottom: 16 }}>
        ...
        <div style={{ marginTop: 16 }}><Btn onClick={generateQuestions} disabled={loading}>{loading ? <Loader2 size={18} style={{ animation: "accspin 1s linear infinite" }} /> : <Sparkles size={18} />} بدء المقابلة وتوليد الأسئلة</Btn></div>
      </Card>
      ...
      {questions.length > 0 && !report && (
        <Card style={{ marginBottom: 16 }}>
          ...
          <Btn variant="gold" onClick={evaluate} disabled={evaluating}>{evaluating ? <Loader2 size={18} style={{ animation: "accspin 1s linear infinite" }} /> : <ClipboardCheck size={18} />} تقييم الإجابات وإصدار التقرير</Btn>
        </Card>
      )}
...
export default AccreditationOS;
